...
Popup {
  ...
  Page {
    ...
    header: QfPageHeader {
      title: qsTr('SmartCloud')

      showBackButton: true
      showCancelButton: false
      showApplyButton: false

      busyIndicatorState: cloudConnection.status === SmartCloudConnection.Connecting || cloudProjectsModel.currentProjectData.Status === SmartCloudProjectsModel.Uploading || cloudProjectsModel.currentProjectData.Status === SmartCloudProjectsModel.Downloading ? 'on' : 'off'
    ...
    ColumnLayout {
      ...
      Text {
        ...
        text: qsTr('The current project is not stored on SmartCloud.<br><br>') + qsTr('Storing projects on SmartCloud offers seamless synchronization, offline editing, and team management.<br><br>') + ' <a href="https://smartfield.cloud/">' + qsTr('Learn more about SmartCloud') + '</a>.'
    ...
    ScrollView {
      ...
      GridLayout {
          ...
          Rectangle {
            ...
            Image {
              ...
              MouseArea {
                ...
                onClicked: {
                  if (cloudConnection.status !== SmartCloudConnection.LoggedIn || cloudProjectsModel.currentProjectData.Status !== SmartCloudProjectsModel.Idle)
                    return;
                  if (!connectionSettings.visible) {
        ...
        Text {
          ...
          text: qsTr('This SmartCloud project was first downloaded with another cloud account. Please sign in with the original account for this project to use the SmartCloud functionality.')
        ...
        Text {
          id: statusText
          visible: cloudProjectsModel.currentProjectData.Status === SmartCloudProjectsModel.Downloading || cloudProjectsModel.currentProjectData.Status === SmartCloudProjectsModel.Uploading
          font: Theme.tipFont
          color: Theme.secondaryTextColor
          text: switch (cloudProjectsModel.currentProjectData.Status) {
          case SmartCloudProjectsModel.Downloading:
            if (cloudProjectsModel.currentProjectData.PackagingStatus === SmartCloudProjectsModel.PackagingBusyStatus) {
              return qsTr('SmartCloud is packaging the latest data just for you; this might take some time, please hold tight');
            } else {
              if (cloudProjectsModel.currentProjectData.PackagingStatus === SmartCloudProjectsModel.PackagingFinishedStatus || cloudProjectsModel.currentProjectData.DownloadProgress > 0.0) {
                ...
                return qsTr('Reaching out to SmartCloud to download project');
              }
            }
          case SmartCloudProjectsModel.Uploading:
            ...
            default:
              return qsTr('SmartCloud is applying the latest uploaded changes. This might take some time, please hold tight…');
            }
          default:
            '';
          }
        ...
        Rectangle {
          ...
          Image {
            ...
            case SmartCloudProjectsModel.Downloading:
              ...
              case SmartCloudProjectsModel.PackagingFinishedStatus || cloudProjectsModel.currentProjectData.DownloadProgress > 0.0:
          ...
          ProgressBar {
            ...
            width: parent.width
            height: 6
            indeterminate: cloudProjectsModel.currentProjectData.PackagingStatus !== SmartCloudProjectsModel.PackagingFinishedStatus && cloudProjectsModel.currentProjectData.DownloadProgress === 0.0
            value: cloudProjectsModel.currentProjectData.DownloadProgress ? cloudProjectsModel.currentProjectData.DownloadProgress : 0
            visible: cloudProjectsModel.currentProjectData.Status === SmartCloudProjectsModel.ProjectStatus.Downloading
          }
        }
        ...
        QfCollapsibleMessage {
          ...
          visible: hasError && cloudProjectsModel.currentProjectData.Status === SmartCloudProjectsModel.Idle && !connectionSettings.visible
        ...
        GridLayout {
          ...
          width: parent.width
          visible: !connectionSettings.visible && cloudProjectsModel.currentProjectData.Status === SmartCloudProjectsModel.Idle
          columns: 1
          ...
          Text {
            ...
            color: Theme.secondaryTextColor
            visible: !cloudProjectsModel.layerObserver.deltaFileWrapper.hasError()
            text: qsTr('Synchronize the whole project with all modified features and download the freshly updated project with all the applied changes from SmartCloud.')
            wrapMode: Text.WordWrap
            horizontalAlignment: Text.AlignHCenter
            Layout.bottomMargin: 20
            Layout.fillWidth: true
          ...
          Text {
            ...
            MouseArea {
              anchors.fill: parent
              onClicked: {
                smartfieldCloudDeltaHistory.open();
              }
            }
          }
        }
        ...
        ColumnLayout {
          ...
          visible: visibility || (cloudProjectsModel.currentProjectId && cloudConnection.status !== SmartCloudConnection.LoggedIn)
  ...
  function show() {
    visible = !visible;
    if (cloudProjectsModel.currentProjectId && cloudConnection.hasToken && cloudConnection.status === SmartCloudConnection.Disconnected) {
