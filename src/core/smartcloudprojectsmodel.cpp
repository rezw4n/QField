...
SmartCloudProjectsModel::SmartCloudProjectsModel()
  : mProject( QgsProject::instance() )
{
  ...
  connect( this, &SmartCloudProjectsModel::currentProjectIdChanged, this, [=]() {
  ...
  connect( this, &SmartCloudProjectsModel::modelReset, this, [=]() {
  ...
  connect( this, &SmartCloudProjectsModel::dataChanged, this, [=]( const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles ) {
  ...
  connect( this, &SmartCloudProjectsModel::cloudConnectionChanged, this, [=]() {
    if ( !mCloudConnection )
      return;

    mUsername = mCloudConnection->username();
    if ( mCloudConnection->status() != SmartCloudConnection::ConnectionStatus::LoggedIn )
    {
      QJsonArray projects;
      reload( projects );
    }
    connect( mCloudConnection, &SmartCloudConnection::usernameChanged, this, [=]() {
      mUsername = mCloudConnection->username();
    } );
  } );
}

SmartCloudConnection *SmartCloudProjectsModel::cloudConnection() const
{
  return mCloudConnection;
}

void SmartCloudProjectsModel::setCloudConnection( SmartCloudConnection *cloudConnection )
{
  if ( mCloudConnection == cloudConnection )
    return;

  if ( cloudConnection )
    connect( cloudConnection, &SmartCloudConnection::statusChanged, this, &SmartCloudProjectsModel::connectionStatusChanged );

  mCloudConnection = cloudConnection;
  emit cloudConnectionChanged();
}

LayerObserver *SmartCloudProjectsModel::layerObserver() const
{
  return mLayerObserver;
}

void SmartCloudProjectsModel::setLayerObserver( LayerObserver *layerObserver )
{
  ...
  connect( layerObserver, &LayerObserver::layerEdited, this, &SmartCloudProjectsModel::layerObserverLayerEdited );
...
QString SmartCloudProjectsModel::currentProjectId() const
{
  return mCurrentProjectId;
}

void SmartCloudProjectsModel::setCurrentProjectId( const QString &currentProjectId )
{
  if ( currentProjectId != QString() )
  {
    const bool forceAutoPush = mProject->readBoolEntry( QStringLiteral( "smartfieldsync" ), QStringLiteral( "forceAutoPush" ), false );
    if ( forceAutoPush )
    {
      projectSetForceAutoPush( currentProjectId, true );
      projectSetAutoPushEnabled( currentProjectId, true );
      projectSetAutoPushIntervalMins( currentProjectId, mProject->readNumEntry( QStringLiteral( "smartfieldsync" ), QStringLiteral( "forceAutoPushIntervalMins" ) ) );
    }
    else
    {
...
QVariantMap SmartCloudProjectsModel::currentProjectData() const
{
  if ( mCurrentProjectId.isEmpty() )
  {
    return QVariantMap();
...
QSet<QString> SmartCloudProjectsModel::busyProjectIds() const
{
  QSet<QString> result;
...
QVariantMap SmartCloudProjectsModel::getProjectData( const QString &projectId ) const
{
  QVariantMap data;
  ...
  for ( auto [key, value] : smartfield::asKeyValueRange( rn ) )
  {
...
void SmartCloudProjectsModel::refreshProjectsList( bool shouldRefreshPublic )
{
  switch ( mCloudConnection->status() )
  {
    case SmartCloudConnection::ConnectionStatus::LoggedIn:
    {
      ...
      connect( reply, &NetworkReply::finished, this, &SmartCloudProjectsModel::projectListReceived );
      break;
    }
    case SmartCloudConnection::ConnectionStatus::Disconnected:
    {
    ...
    case SmartCloudConnection::ConnectionStatus::Connecting:
      // Nothing done for this intermediary status.
      break;
  }
}

QModelIndex SmartCloudProjectsModel::findProjectIndex( const QString &projectId ) const
{
...
SmartCloudProjectsModel::CloudProject *SmartCloudProjectsModel::findProject( const QString &projectId ) const
{
...
void SmartCloudProjectsModel::removeLocalProject( const QString &projectId )
{
  QDir dir( QStringLiteral( "%1/%2/%3" ).arg( SmartCloudUtils::localCloudDirectory(), mUsername, projectId ) );
  ...
  QSettings().remove( QStringLiteral( "SmartCloud/projects/%1" ).arg( projectId ) );
}

SmartCloudProjectsModel::ProjectStatus SmartCloudProjectsModel::projectStatus( const QString &projectId ) const
{
  CloudProject *project = findProject( projectId );

  if ( !project )
    return SmartCloudProjectsModel::ProjectStatus::Idle;

  return project->status;
}

bool SmartCloudProjectsModel::canSyncProject( const QString &projectId ) const
{
  CloudProject *project = findProject( projectId );
...
SmartCloudProjectsModel::ProjectModifications SmartCloudProjectsModel::projectModification( const QString &projectId ) const
{
  CloudProject *project = findProject( projectId );
...
void SmartCloudProjectsModel::refreshProjectModification( const QString &projectId )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  if ( !projectIndex.isValid() )
    return;
...
void SmartCloudProjectsModel::refreshProjectFileOutdatedStatus( const QString &projectId )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  if ( !projectIndex.isValid() )
    return;
  ...
  connect( reply, &NetworkReply::finished, reply, [=]() {
    ...
    {
      QgsLogger::debug( QStringLiteral( "Project %1: failed to refresh the project file outdated satus. %2" ).arg( projectId, SmartCloudConnection::errorString( rawReply ) ) );
      return;
    }

    const QJsonArray files = QJsonDocument::fromJson( rawReply->readAll() ).array();
    const QString lastProjectFileMd5 = SmartCloudUtils::projectSetting( project->id, QStringLiteral( "lastProjectFileMd5" ), QString() ).toString();
    for ( const auto file : files )
    {
      ...
      if ( fileName.endsWith( QStringLiteral( ".qgs" ) ) || fileName.endsWith( QStringLiteral( ".qgz" ) ) )
      {
        if ( lastProjectFileMd5.isEmpty() )
        {
          ...
          SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "lastProjectFileMd5" ), fileDetails.value( "md5sum" ).toString() );
        }
        else if ( lastProjectFileMd5 != fileDetails.value( "md5sum" ).toString() )
        {
          project->projectFileIsOutdated = true;
          SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "projectFileOudated" ), true );
...
QString SmartCloudProjectsModel::layerFileName( const QgsMapLayer *layer ) const
{
  return layer->dataProvider()->dataSourceUri().split( '|' )[0];
}

SmartCloudProjectsModel::JobStatus SmartCloudProjectsModel::getJobStatusFromString( const QString &status ) const
{
...
QString SmartCloudProjectsModel::getJobTypeAsString( JobType jobType ) const
{
  switch ( jobType )
  {
...
void SmartCloudProjectsModel::projectCancelDownload( const QString &projectId )
{
  if ( !mCloudConnection )
    return;
...
void SmartCloudProjectsModel::projectRefreshData( const QString &projectId, const ProjectRefreshReason &refreshReason )
{
  if ( !mCloudConnection )
    return;
  ...
  connect( reply, &NetworkReply::finished, reply, [=]() {
    ...
    if ( rawReply->error() != QNetworkReply::NoError )
    {
      emit projectRefreshed( projectId, refreshReason, SmartCloudConnection::errorString( rawReply ) );
      return;
    }
    ...
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "name" ), project->name );
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "owner" ), project->owner );
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "description" ), project->description );
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "userRole" ), project->userRole );
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "userRoleOrigin" ), project->userRoleOrigin );
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "isPrivate" ), project->isPrivate );
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "canRepackage" ), project->canRepackage );
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "needsRepackaging" ), project->needsRepackaging );
...
void SmartCloudProjectsModel::projectStartJob( const QString &projectId, const JobType jobType )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  if ( !projectIndex.isValid() )
    return;
  ...
  connect( reply, &NetworkReply::finished, reply, [=]() {
    ...
    if ( rawReply->error() != QNetworkReply::NoError )
    {
      QString errorString = SmartCloudConnection::errorString( rawReply );
...
void SmartCloudProjectsModel::projectGetJobStatus( const QString &projectId, const JobType jobType )
{
  CloudProject *project = findProject( projectId );
...
void SmartCloudProjectsModel::projectPackageAndDownload( const QString &projectId )
{
  ...
  auto projectRepackageIfNeededAndThenDownload = [=]() {
    if ( project->needsRepackaging )
    {
      ...
      connect( this, &SmartCloudProjectsModel::projectJobFinished, tempProjectJobFinishedParent, [=]( const QString &jobProjectId, const JobType jobType, const QString &errorString ) {
       ...
       || project->lastRefreshedAt.secsTo( QDateTime::currentDateTimeUtc() ) > CACHE_PROJECT_DATA_SECS )
  {
    ...
    connect( this, &SmartCloudProjectsModel::projectRefreshed, tempProjectRefreshParent, [=]( const QString &refreshedProjectId, const ProjectRefreshReason refreshReason, const QString &errorString ) {
  ...
  connect( this, &SmartCloudProjectsModel::projectDownloadFinished, tempProjectDownloadFinishedParent, [=]( const QString &finishedProjectId, const QString &errorString ) {
...
void SmartCloudProjectsModel::projectDownload( const QString &projectId )
{
  ...
  connect( reply, &NetworkReply::finished, reply, [=]() {
    ...
    if ( rawReply->error() != QNetworkReply::NoError )
    {
      QgsLogger::debug( QStringLiteral( "Project %1: failed to get latest package data. %2" ).arg( projectId, SmartCloudConnection::errorString( rawReply ) ) );
    ...
    for ( const QJsonValue &fileValue : files )
    {
      ...
      QString projectFileName = QStringLiteral( "%1/%2/%3/%4" ).arg( SmartCloudUtils::localCloudDirectory(), mUsername, projectId, fileName );
...
void SmartCloudProjectsModel::updateActiveProjectFilesToDownload( const QString &projectId )
{
  if ( !mCloudConnection )
    return;
...
void SmartCloudProjectsModel::projectDownloadFiles( const QString &projectId )
{
...
bool SmartCloudProjectsModel::projectMoveDownloadedFilesToPermanentStorage( const QString &projectId )
{
  if ( !mCloudConnection )
    return false;
  CloudProject *project = findProject( projectId );
  ...
  for ( const QString &fileName : fileNames )
  {
    ...
    QDir dir( QStringLiteral( "%1/%2/%3/%4" ).arg( SmartCloudUtils::localCloudDirectory(), mUsername, projectId, fileInfo.path() ) );
...
void SmartCloudProjectsModel::projectUpload( const QString &projectId, const bool shouldDownloadUpdates )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  ...
  const QFileInfo projectInfo( SmartCloudUtils::localProjectFilePath( mUsername, projectId ) );
  ...
  for ( const QString &fileName : attachmentFileNames )
  {
    ...
    SmartCloudUtils::addPendingAttachment( project->id, absoluteFilePath );
  }
  ...
  connect( deltasCloudReply, &NetworkReply::finished, this, [=]() {
    ...
    {
      project->deltaFileUploadStatusString = SmartCloudConnection::errorString( deltasReply );
  ...
  connect( this, &SmartCloudProjectsModel::networkDeltaUploaded, networkDeltaUploadedParent, [=]( const QString &uploadedProjectId ) {
    ...
    else
    {
      ...
      SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "lastLocalPushDeltas" ), project->lastLocalPushDeltas );
  ...
  connect( this, &SmartCloudProjectsModel::networkDeltaStatusChecked, networkDeltaStatusCheckedParent, [=]( const QString &uploadedProjectId ) {
    if ( projectId != uploadedProjectId )
      return;
  ...
  //  // //////////
  //  connect( this, &SmartCloudProjectsModel::networkAllLayersDownloaded, this, [ = ]( const QString & callerProjectId )
  //  {
...
void SmartCloudProjectsModel::refreshProjectDeltaList( const QString &projectId )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  if ( !projectIndex.isValid() )
    return;
...
void SmartCloudProjectsModel::projectGetDeltaStatus( const QString &projectId )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  if ( !projectIndex.isValid() )
    return;
  ...
  connect( deltaStatusReply, &NetworkReply::finished, this, [=]() {
    ...
    if ( rawReply->error() != QNetworkReply::NoError )
    {
      ...
      project->deltaFileUploadStatusString = SmartCloudConnection::errorString( rawReply );
...
void SmartCloudProjectsModel::projectCancelUpload( const QString &projectId )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  if ( !projectIndex.isValid() )
    return;
...
void SmartCloudProjectsModel::connectionStatusChanged()
{
  refreshProjectsList();
}

void SmartCloudProjectsModel::layerObserverLayerEdited( const QString &layerId )
{
  Q_UNUSED( layerId )
  CloudProject *project = findProject( mCurrentProjectId );
...
void SmartCloudProjectsModel::projectListReceived()
{
  ...
  if ( rawReply->error() != QNetworkReply::NoError )
  {
    emit warning( SmartCloudConnection::errorString( rawReply ) );
    return;
  }
...
NetworkReply *SmartCloudProjectsModel::downloadFile( const QString &projectId, const QString &fileName )
{
...
void SmartCloudProjectsModel::downloadFileConnections( const QString &projectId, const QString &fileName )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  ...
  connect( reply, &NetworkReply::redirected, reply, [=]( const QUrl &url ) {
    ...
    // we need to somehow finish the request, otherwise it will remain unfinished for the SmartCloudConnection
    reply->abort();
  ...
  connect( reply, &NetworkReply::finished, reply, [=]() {
    ...
    {
      hasError = true;
      errorMessageDetail = SmartCloudConnection::errorString( rawReply );
    ...
    if ( hasError )
    {
      ...
      const QString trimmedMessage = baseMessage + QStringLiteral( " " ) + tr( "System message: " )
                                     + ( ( errorMessageDetail.size() > 100 )
                                           ? ( errorMessageDetail.left( 100 ) + tr( " (see more in the SmartField error log)…" ) )
                                           : errorMessageDetail );
    ...
    if ( project->downloadFilesFinished == fileNames.count() )
    {
      ...
      if ( !hasError )
      {
        ...
        project->localPath = SmartCloudUtils::localProjectFilePath( mUsername, projectId );
        ...
        SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "lastExportedAt" ), project->lastExportedAt );
        SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "lastExportId" ), project->lastExportId );
        SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "lastLocalExportedAt" ), project->lastLocalExportedAt );
        SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "lastLocalExportId" ), project->lastLocalExportId );
        SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "lastLocalDataLastUpdatedAt" ), project->lastLocalDataLastUpdatedAt );
        SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "lastProjectFileMd5" ), QString() );
        SmartCloudUtils::setProjectSetting( projectId, QStringLiteral( "projectFileOudated" ), false );
...
QHash<int, QByteArray> SmartCloudProjectsModel::roleNames() const
{
...
void SmartCloudProjectsModel::projectSetForceAutoPush( const QString &projectId, bool force )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
...
void SmartCloudProjectsModel::projectSetAutoPushEnabled( const QString &projectId, bool enabled )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  ...
  if ( projectIndex.isValid() )
  {
    ...
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "autoPushEnabled" ), project->autoPushEnabled );
    emit dataChanged( projectIndex, projectIndex, QVector<int>() << AutoPushEnabledRole );
  }
}

void SmartCloudProjectsModel::projectSetAutoPushIntervalMins( const QString &projectId, int minutes )
{
  const QModelIndex projectIndex = findProjectIndex( projectId );
  ...
  if ( projectIndex.isValid() )
  {
    ...
    SmartCloudUtils::setProjectSetting( project->id, QStringLiteral( "autoPushIntervalMins" ), project->autoPushIntervalMins );
    emit dataChanged( projectIndex, projectIndex, QVector<int>() << AutoPushIntervalMinsRole );
  }
}

void SmartCloudProjectsModel::reload( const QJsonArray &remoteProjects )
{
  beginResetModel();
  mProjects.clear();
  ...
  auto restoreLocalSettings = [=]( CloudProject *cloudProject, const QDir &localPath ) {
    ...
    cloudProject->lastExportId = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "lastExportId" ) ).toString();
    cloudProject->lastExportedAt = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "lastExportedAt" ) ).toString();
    cloudProject->lastLocalExportId = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "lastLocalExportId" ) ).toString();
    cloudProject->lastLocalExportedAt = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "lastLocalExportedAt" ) ).toString();
    cloudProject->lastLocalPushDeltas = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "lastLocalPushDeltas" ) ).toString();
    cloudProject->lastLocalDataLastUpdatedAt = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "lastLocalDataLastUpdatedAt" ) ).toDateTime();
    cloudProject->isOutdated = cloudProject->dataLastUpdatedAt > cloudProject->lastLocalDataLastUpdatedAt;
    cloudProject->projectFileIsOutdated = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "projectFileOudated" ), false ).toBool();
    cloudProject->autoPushEnabled = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "autoPushEnabled" ), false ).toBool();
    cloudProject->autoPushIntervalMins = SmartCloudUtils::projectSetting( cloudProject->id, QStringLiteral( "autoPushIntervalMins" ), 30 ).toInt();

    // generate local export id if not present. Possible reasons for missing localExportId are:
    // - just upgraded SmartField that introduced the field
    ...
    if ( cloudProject->lastLocalExportId.isEmpty() )
    {
      ...
      SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "lastLocalExportId" ), cloudProject->lastLocalExportId );
    }
  };
  ...
  for ( const auto project : remoteProjects )
  {
    ...
    SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "owner" ), cloudProject->owner );
    SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "name" ), cloudProject->name );
    SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "description" ), cloudProject->description );
    SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "userRole" ), cloudProject->userRole );
    SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "userRoleOrigin" ), cloudProject->userRoleOrigin );
    SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "canRepackage" ), cloudProject->canRepackage );
    SmartCloudUtils::setProjectSetting( cloudProject->id, QStringLiteral( "needsRepackaging" ), cloudProject->needsRepackaging );

    if ( !mUsername.isEmpty() )
    {
      cloudProject->localPath = SmartCloudUtils::localProjectFilePath( mUsername, cloudProject->id );
      QDir localPath( QStringLiteral( "%1/%2/%3" ).arg( SmartCloudUtils::localCloudDirectory(), mUsername, cloudProject->id ) );
      if ( localPath.exists() )
      {
  ...
  QDirIterator userDirs( SmartCloudUtils::localCloudDirectory(), QDir::Dirs | QDir::NoDotAndDotDot );
  while ( userDirs.hasNext() )
  {
    ...
    QDirIterator projectDirs( QStringLiteral( "%1/%2" ).arg( SmartCloudUtils::localCloudDirectory(), username ), QDir::Dirs | QDir::NoDotAndDotDot );
    while ( projectDirs.hasNext() )
    {
      ...
      const QString owner = SmartCloudUtils::projectSetting( projectId, QStringLiteral( "owner" ) ).toString();
      const QString name = SmartCloudUtils::projectSetting( projectId, QStringLiteral( "name" ) ).toString();
      const QString description = SmartCloudUtils::projectSetting( projectId, QStringLiteral( "description" ) ).toString();
      const QString updatedAt = SmartCloudUtils::projectSetting( projectId, QStringLiteral( "updatedAt" ) ).toString();
      const QString userRole = SmartCloudUtils::projectSetting( projectId, QStringLiteral( "userRole" ) ).toString();
      const QString userRoleOrigin = SmartCloudUtils::projectSetting( projectId, QStringLiteral( "userRoleOrigin" ) ).toString();
      ...
      cloudProject->localPath = SmartCloudUtils::localProjectFilePath( username, cloudProject->id );
      QDir localPath( QStringLiteral( "%1/%2/%3" ).arg( SmartCloudUtils::localCloudDirectory(), username, cloudProject->id ) );
      restoreLocalSettings( cloudProject, localPath );
...
int SmartCloudProjectsModel::rowCount( const QModelIndex &parent ) const
{
  if ( !parent.isValid() )
...
QVariant SmartCloudProjectsModel::data( const QModelIndex &index, int role ) const
{
...
bool SmartCloudProjectsModel::revertLocalChangesFromCurrentProject()
{
  CloudProject *project = findProject( mCurrentProjectId );
...
bool SmartCloudProjectsModel::discardLocalChangesFromCurrentProject()
{
  CloudProject *project = findProject( mCurrentProjectId );
  if ( !project )
    return false;
...
void SmartCloudProjectsModel::setGpkgFlusher( QgsGpkgFlusher *flusher )
{
...
bool SmartCloudProjectsModel::deleteGpkgShmAndWal( const QStringList &gpkgFileNames )
{
  bool isSuccess = true;
...
QStringList SmartCloudProjectsModel::filterGpkgFileNames( const QStringList &fileNames ) const
{
  QStringList gpkgFileNames;
...
QStringList SmartCloudProjectsModel::projectFileNames( const QString &projectPath, const QStringList &fileNames ) const
{
...
SmartCloudProjectsFilterModel::SmartCloudProjectsFilterModel( QObject *parent )
  : QSortFilterProxyModel( parent )
{
  sort( 0 );
}

void SmartCloudProjectsFilterModel::setProjectsModel( SmartCloudProjectsModel *projectsModel )
{
...
SmartCloudProjectsModel *SmartCloudProjectsFilterModel::projectsModel() const
{
  return mSourceModel;
}

void SmartCloudProjectsFilterModel::setFilter( ProjectsFilter filter )
{
  if ( mFilter == filter )
    return;
...
SmartCloudProjectsFilterModel::ProjectsFilter SmartCloudProjectsFilterModel::filter() const
{
  return mFilter;
}

void SmartCloudProjectsFilterModel::setShowLocalOnly( bool showLocalOnly )
{
...
bool SmartCloudProjectsFilterModel::showLocalOnly() const
{
  return mShowLocalOnly;
}

bool SmartCloudProjectsFilterModel::filterAcceptsRow( int source_row, const QModelIndex &source_parent ) const
{
  if ( mShowLocalOnly && mSourceModel->data( mSourceModel->index( source_row, 0, source_parent ), SmartCloudProjectsModel::LocalPathRole ).toString().isEmpty() )
  {
    return false;
  }
  ...
  switch ( mFilter )
  {
    case PrivateProjects:
      ...
      ok = mSourceModel->data( mSourceModel->index( source_row, 0, source_parent ), SmartCloudProjectsModel::UserRoleOriginRole ).toString() != QStringLiteral( "public" )
           || !mSourceModel->data( mSourceModel->index( source_row, 0, source_parent ), SmartCloudProjectsModel::LocalPathRole ).toString().isEmpty();
      break;
    case PublicProjects:
      ok = mSourceModel->data( mSourceModel->index( source_row, 0, source_parent ), SmartCloudProjectsModel::UserRoleOriginRole ).toString() == QStringLiteral( "public" );
      break;
